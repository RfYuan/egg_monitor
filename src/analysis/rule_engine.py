...
def run_all_checks():
    ...
    db = SessionLocal()
    try:
        quotes = get_futures_quotes(db, "JD2609", limit=1)
        if not quotes:
            log.warning("No futures data for rule check")
            return
        ...
        for rule in rules:
            try:
                if check_price_rule(rule, latest_price):
                    log.info(f"Rule {rule['id']} triggered!")
                    send_notification(
                        title=rule["name"],
                        content=rule["action"]["message"],
                        level=rule["level"],
                        rule_id=rule["id"],
                        data_snapshot={"price": latest_price}
                    )
            except Exception as e:
